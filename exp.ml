...
let priority = function
  | Match _ | Let _ | Fun _ | LetRec _ -> 0
  | If _ -> 10
  | Cons _ -> 20
  | BinOp (_, Prim.Lt, _) -> 30
  | BinOp (_, Prim.Plus, _) | BinOp (_, Prim.Minus, _) -> 40
  | BinOp (_, Prim.Times, _) -> 50
  | App _ -> 60
  | Int _ | Bool _ | Var _ | Nil -> 70
...
let rec to_string_aux prio exp = 
  ...
  | Let (x, e1, e2) ->
      "let" ^. x ^. "=" ^. to_string_nonassoc e1 ^. "in" ^.
      to_string_assoc e2
  | Fun (x, e) ->
  ...
  | Nil -> "[]"
  | Cons (e1, e2) ->
      to_string_nonassoc e1 ^. "::" ^. to_string_assoc e2
  | Match (e1, e2, x, y, e3) ->
      "match" ^. to_string_assoc e1 ^. "with" ^.
      "[] ->" ^. to_string_assoc e2 ^.
      "|" ^. x ^. "::" ^. y ^. "->" ^. to_string_assoc e3)
